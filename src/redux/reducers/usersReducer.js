...
export const usersReducer = (state = { currentUser: null, role: null,exp:null }, action) => {
    switch (action.type) {//switching action type/name that was dispatched
        ...
        case 'USER_LOGIN_SUCCESS'://if success loading false, and set userInfo to action.payload which is user data from dispatched action set as payload
            return { ...state, loading: false, currentUser: action.payload.token }//if success action will have user data on it as payload
        case 'USER_LOGIN_FAIL':
            return { loading: false, error: action.payload }//if fail dispatched action will have error data on it. as payload
        case 'USER_LOGOUT':
            return { ...state, currentUser: null, role: null,exp: null }//if dispatched action type/name is USER_LOGOUT return empty object
